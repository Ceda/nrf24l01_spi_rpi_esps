...
int spi_read_reg(int reg){
   char cmdstr[] = {reg, 0}; //need second byte to complete transfer
   bb_spi_xfer(pi, CE0, cmdstr, (char *)inBuf, 2); 
   return inBuf[1];
}
...
int main(int argc, char *argv[])
{
    ...
    if ((pi = pigpio_start(0, 0)) < 0) { fprintf(stderr, "pigpio init failed (%d).\n", pi); }
    ...
    while(1) {
        ...
        spi_write_reg ( 0xe1, 0x00); //flush rx fifo
        ...
        for(n = 0; n < 32; n++) {printf(" 0x%02x", data[n+1]); if(n%16 == 15) printf("\n      ");
        }
        printf ("\n");
